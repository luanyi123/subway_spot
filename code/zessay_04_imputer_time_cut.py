...
for file in train_files:
    ...
    try:
        ...
        to_merge = train_df.drop(['time_cut', 'inNums', 'outNums'], axis=1)
        ...
        train_df = train_df.merge(to_merge, on=['stationID'], how='left').drop_duplicates().reset_index(drop=True)
        train_df.to_csv("./train03_21/02/" + file, index=False)
        scount += 1
        print(file, "处理成功！")
    except:
        fcount += 1
