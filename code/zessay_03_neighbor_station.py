...
for index in columns:
    roadmap[int(index)] = np.where(mapdata[index] == 1)
...
station_type = pd.DataFrame(data={"stationID": stationID,
                                  "station_type": [len(a) for arr in neighbor_station for a in arr ],
                                 "neighbor_station": [a for arr in neighbor_station for a in arr]}
                           )
...
for file in train_files:
    ...
    try:
        ...
        train_df.to_csv("./train03_21/" + file, index=False)
        scount += 1
        print(file, "处理成功！")
    except:
        fcount += 1
